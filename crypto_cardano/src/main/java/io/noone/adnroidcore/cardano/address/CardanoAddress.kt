            ...
            if (field.isEmpty()) {
                field = when (type) {
                    AddressType.DELEGATE -> {
                        cardanoMainNetHRP + cardanoPrefixSplitSimbol + Bech32.encode(
                            byteArrayOf(
                                prefix
                            ) + payload, cardanoMainNetHRP
                        )
                    }
                    AddressType.STAKE -> {
                        cardanoMainNetStakeHRP + cardanoPrefixSplitSimbol + Bech32.encode(
                            byteArrayOf(prefix) + payload,
                            cardanoMainNetStakeHRP
                        )
                    }
                }
            }
