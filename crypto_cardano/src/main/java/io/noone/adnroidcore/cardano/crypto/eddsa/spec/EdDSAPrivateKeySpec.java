 ...
 *
 */
package io.noone.adnroidcore.cardano.crypto.eddsa.spec;
...
import io.noone.adnroidcore.cardano.crypto.eddsa.math.GroupElement;
...
public class EdDSAPrivateKeySpec implements KeySpec {
    ...
    /**
     *  Initialize directly from the hash/extended key.
     *  getSeed() will return null if this constructor is used.
     *
     *  @param spec the parameter specification for this key
     *  @param h the private key (64 bytes for Ed25519)
     *  @param clamp if true, apply standard Ed25519 clamping; if false, use h as-is (for Cardano/BIP32-Ed25519)
     *  @throws IllegalArgumentException if hash length is wrong
     */
    public EdDSAPrivateKeySpec(EdDSAParameterSpec spec, byte[] h, boolean clamp) {
        if (h.length != spec.getCurve().getField().getb()/4)
            throw new IllegalArgumentException("hash length is wrong");

        this.seed = null;
        this.h = Arrays.copyOf(h, h.length); // Don't modify the input array
        this.spec = spec;
        int b = spec.getCurve().getField().getb();
        ...
        a = Arrays.copyOfRange(this.h, 0, b/8);
