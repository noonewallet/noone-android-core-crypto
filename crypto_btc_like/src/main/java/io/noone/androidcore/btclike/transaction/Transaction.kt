...
class Transaction(networkParameters: NetworkParameters) {

    class HashBuilder(private val hashType: HashType) {
        ...
        override fun toString(): String {
            ...
            return when (hashType) {
                HashType.REVERSED -> payloadByteArray.sha256sha256.reversedArray()
                HashType.DEFAULT -> payloadByteArray.sha256sha256
            }.hex
        }
    }
    ...
    private val hashBuilder: HashBuilder = HashBuilder(networkParameters.hashType)
    ...
    val transactionInfo: String
        get() {
            ...
            return StringBuilder()
                .appendLine((if (realSize == newSize) "" else "Virtual ") + "Size (bytes):")
                .appendLine(size)
                .appendLine("Fee (satoshi/byte):")
                .appendLine(String.format("%.3f", fee / size.toDouble()))
                .toString()
        }
    ...
    internal fun addData(
        ...
        split.appendLine(value)
    }

    internal fun addHeader(part: TxPart) {
        split.appendLine(part.alias)
    }
